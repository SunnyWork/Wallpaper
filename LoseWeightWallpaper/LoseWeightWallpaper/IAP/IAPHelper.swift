...
public class IAPHelper: NSObject {
  ...
  let bought = Variable(DataContainer.shared.bought)
  private var observerAdded = false
  
  var saleProduct: SKProduct?
  
  func getAmount() -> String? {
    if let p = saleProduct {
  ...
  func checkProductAlreadyBuy() {
    guard !DataContainer.shared.bought else { return }
    
    restorePurchases(){(productIdentifiers, error) in
      var buy = false
      for one in productIdentifiers {
        if one == Constants.productId {
          buy = true
          DataContainer.shared.bought = true
          self.bought.value = true
          break
        }
      }
      if !buy {
  ...
  func getProductList() {
    ...
    productIdentifiers.insert(Constants.productId)
    
    IAP.requestProducts(productIdentifiers) { (response, error) in
      if let products = response?.products, !products.isEmpty {
        for product in products {
          self.saleProduct = product
        }
      }
    }
  }
  
  func purchase(callback: @escaping ((Bool) -> Void)) {
    let productIdentifier = Constants.productId
