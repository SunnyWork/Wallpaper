...
class EditAlertViewController: BaseViewController, UITableViewDelegate, UITableViewDataSource {
    ...
    func modify_NavBar() -> Void
    {
        ...
        if ( editAlertFlag == type_modify_alert.edit_alert.rawValue ){
            ...
            self.btnRight?.setTitle("Save", for: UIControlState.normal)
        }
    ...
    func init_data() -> Void
    {
        ...
        self.arrayTitle = ["Repeat", "Tag"]
        
        if dataModel == nil {
            self.noticeRepeat = "0"
            self.noticeTab = "Sport time!"
        }
        else {
    ...
    func layoutCell(tableView: UITableView, indexPath: IndexPath) -> UITableViewCell
    {
        ...
        if ( indexPath.section == 0 ){
            ...
            let labelTitle : UILabel = UILabel.init(frame: CGRect(x: Int(spacingLayoutLeft), y: 0, width: 200, height: cellHeight))
            ...
            let labelValue : UILabel = UILabel.init(frame: CGRect(x: Int(spacingLayoutLeft + 80), y: 0, width: (Int(screenWidth) - Int(spacingLayoutLeft + 80) - 30), height: cellHeight))
